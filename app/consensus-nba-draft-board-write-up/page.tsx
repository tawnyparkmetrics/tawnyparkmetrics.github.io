    ...
    const sections = [
        ...
        },
        {
            ...
            content: (
                <div>
                    ...
                    <p className="mb-4">
                        <strong>Redraft + Pick Value</strong> is far more subjective. Currently, this redraft order is determined internally by TPM. However, moving forward, we will transition to a community redraft (based on input from site-visitors). Ultimately, this criteria is more closely related
                        to public-sentiment than EPM or EW ratings alone. While that opinion generates the redraft order, it still requires a scale. For this, we use a normalized combination of Kevin Pelton's & Jacob Goldstein's draft pick value charts. The result is a scale assigning 4000 points to the
                    ...
                    <p className="mb-4">
                        Put simply, we simulate each board as a draft order and calculate the average opportunity cost of every suboptimal selection. To explain the reasoning behind this approach, it's worth outlining the simpler alternatives we first considered and why they fall short.
                    </p>
                    <p className="mb-4">
                        The simplest way to evaluate pre-draft rankings is via some form of <strong>correlation</strong>. Essentially, what was the correlation between the order of each board's rankings and the order of a redraft, based on a select criteria (ex. redraft in the order of career EPM)? While
                        correlation successfully measures the strength of the ranking relationship, it ignores the magnitude of prediction errors.
                    </p>
                    <p className="mb-4">
                    ...
                    <p className="mb-4">
                        The next natural progression is an error-based approach, such as <strong>MAE (mean-average error)</strong>. Here each draft board is treated as a set of predictions about player value. Thus, we can compare, for example, the career avg. EPM of the player ranked first on a board to
                        the career avg. EPM of the true top EPM performer. Accordingly, we can measure "loss" through the scale of each criteria, a clear upgrade over raw-correlation. But, in the words of Lee Corso, not so fast my friend. Take a look at what happens when you actually implement MAE:
                    </p>
                    <ul className="list-disc list-inside space-y-2 ml-4 mb-4">
                        <li>Board ranks LaMelo #1. In the redraft, he's #3. Pick #1 = 4000 points, Pick #3 = 2799.61 points. Loss = 1200.39 (absolute value difference).</li>
                        <li>Board ranks Haliburton #3. In the redraft, he's #1. Pick #3 = 2799.61 points, Pick #1 = 4000 points. Loss = 1200.39 (absolute value difference).</li>
                    </ul>
                    <p className="mb-4">
                        Note, I would've included EPM or EW in this example, but those ratings are exclusive to paid subscribers to the Dunks&Threes website. To clarify, TPM pays for a yearly subscription – enabling us to access EPM & EW ratings for internal analysis, not external distribution.
                    </p>
                    <p className="mb-4">
                        Taking the MAE (the absolute value difference between a player's predicted value and their redraft value) double-counts errors. In practice, this exaggerates the penalty of single mistakes. However, using error as a measure of opportunity cost is, at least, a step in the right direction.
                    </p>
                    <p className="mb-4">
                        We want to answer: how much value did each board "miss" by not ranking (theoretically, selecting) players in the optimal order? In treating each ranking as a draft pick, we also want to avoid penalizing analysts for taking the best remaining player available (avoid double counting errors).
                    </p>
                    <p className="mb-4">
                        The last consideration is whether to measure the opportunity cost of passing on the single best available player, or on all better available players. We chose the latter, as it more closely mirrors how we judge real draft decisions. When a team selects Player A, we'll often consider multiple
                        alternatives (they could've taken Player B, C, D, etc.), both during the draft and years in the future.
                    </p>
                    <p className="mb-4">Accordingly, here's how the opportunity cost calculation works:</p>
                    <ul className="list-disc list-inside space-y-2 ml-4 mb-4">
                    ...
                    <p className="mb-4">
                        The resulting output is the average opportunity cost for each player ranked up to a certain cutoff. For instance, Kevin Pelton's 2020 board missed out on an average of 1.058 career avg. EPM, per player ranked Top-14 (lottery). The generalized way to interpret this output: for every suboptimal pick (i.e. rank), on average, the analysts would've "lost" _____.
                    </p>
                    <p className="mb-4">We incorporate two additional exploratory criteria:</p>
                    <ol className="list-decimal list-inside space-y-2 ml-4 mb-4">
        ...
        },
        {
            ...
            content: (
                <div>
                    <p className="mb-4">
                        The 'Lottery', 'Top 30', and 'Top 60' cutoffs allow us to evaluate boards fairly, despite their varying sizes. As explained in <strong>How are individual boards evaluated?</strong>, we calculate the average opportunity cost of every suboptimal rank, but only up to these cutoffs. This way, we avoid comparing, say, a
        ...
        },
        {
            ...
            content: (
                <div>
                    <p className="mb-4">
                        The consensus evaluation tracks the predictiveness of each draft board, up to the most recent complete NBA season, according to multiple criteria. <strong>Lower average opportunity cost values for EPM, EW, and Redraft + Pick Value indicate that a board's rankings were, in hindsight, more accurate.</strong>
                    </p>
                    <p className="mb-4">
                        It's important to note these results are retrospective and, naturally, depend on the chosen criteria. They should be understood as measures of board predictiveness, not direct evidence of an analyst's process or ability.
                    </p>
                    <p>
                        For a deeper dive into methodology, see <strong>How are individual boards evaluated?</strong> or explore the Evaluation Criteria dropdown on each evaluation page.
                    </p>
                </div>
            )
        },
        {
            ...
            content: (
                <div>
                    <p className="mb-4">
                        Relative to most public boards, <strong>the NBA collectively makes better draft decisions than general narrative suggests.</strong> Acknowledging that NBA teams hold clear advantages (personnel, data, access, etc.), their 1st round performance is 71st percentile since 2020. However, as mentioned in What are the NBA & Consensus
                        boards in evaluation? (where you can find the NBA’s percentile draft performance by year), the impact of draft position on career outcomes does confound these results.</p>
                    <p className="mb-4">
                    ...
                    <p>
                        For now, however, no one has, seemingly, “cracked” the draft. As we already knew, the draft is a problem without a perfect solution (extends to other sports as well). Every analyst has misses, and the data suggests we should all approach this process with humility. That said, the goal of the evaluation is not to discourage analysts; rather, to motivate continued learning, iteration, and refinement of their process.
                    </p>
                    <p>
                    ...
                    <p>
                        <strong>In future work, we hope to analyze multi-year trends and apply clustering techniques, revealing more nuanced findings. Our interest lies less in individual board performance and more in identifying broader patterns: how do analysts naturally group, how do those groups perform, and what can we infer about specific draft approaches?</strong>
                    </p>
                    <p>
        ...
        },
        {
            ...
            content: (
                <div>
                    ...
                    <p>
                        <em>- Percentiles are based on their rank relative to all the individual boards that form the consensus that year</em>
                    </p>
                    <p>
                        <em>- Top 60 in 2022-2024 are N/A for ‘NBA’, since there were less than 60 picks in these draft classes (will substitute with top 58 & top 59 comparisons, where applicable, in a future update)</em>
                    </p>
                    <p>
                        <em>- 2025 are all N/A since there is not any evaluation data (at the time of writing, players from this draft class are yet to play in the NBA)</em>
                    </p>
                    <p>
                        <li><em>The <strong>Correlation % column refers to the similarity between NBA & Consensus</strong></em></li>
                    </p>
                    <p>
                        <em>- Correlation % for ‘Top 60’, in 2022-2024, is really comparing Top 58 (only 58 draft picks – two forfeited)</em>
                    </p>
                    <p>
                        <em>- Correlation % for ‘Top 60’, in 2025, is really comparing Top 59 (only 59 draft picks – one forfeited)</em>
                    </p>
                </div>
            )
        },
